...
function doAjaxQuery(input, link)
{
    ...
    $.ajax({
        ...
        error: function()
        {
            $('#rwform_loader').remove(); 
            rwform_alertbar_div.show();            
            rwform_alertbar_div.addClass('alert-error');            
...
// DATEPICKER
    ...
    var checkin = $('#msg_startdate').datepicker({
    ...
    var checkout = $('#msg_enddate').datepicker({
        onRender: function(date) {
    ...
    $('#msg_startdate').datepicker();
    $('#msg_enddate').datepicker();
